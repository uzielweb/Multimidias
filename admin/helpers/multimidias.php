...
/**
 * Multimidias component helper.
 */
abstract class MultimidiasHelper
{
	/**
	...
	{
		JHtmlSidebar::addEntry(JText::_('Multimidias'), 'index.php?option=com_multimidias&view=multimidias', $submenu == 'multimidias');
		JHtmlSidebar::addEntry(JText::_('Musics'), 'index.php?option=com_multimidias&view=musics', $submenu == 'musics');
		JHtmlSidebar::addEntry(JText::_('Videos'), 'index.php?option=com_multimidias&view=videos', $submenu == 'videos');
		JHtmlSidebar::addEntry(JText::_('Photos'), 'index.php?option=com_multimidias&view=photos', $submenu == 'photos');
		JHtmlSidebar::addEntry(JText::_('Documents'), 'index.php?option=com_multimidias&view=documents', $submenu == 'documents');
		JHtmlSidebar::addEntry(JText::_('Categories'), 'index.php?option=com_categories&view=categories&extension=com_multimidias', $submenu == 'categories');

		// set some global property
		$document = JFactory::getDocument();
		if ($submenu == 'categories'){
			$document->setTitle(JText::_('Categories - Multimidias'));
		};
	}
	...
	public static function getActions($Id = 0)
	{
		...
		if (empty($Id)){
			$assetName = 'com_multimidias';
		} else {
			$assetName = 'com_multimidias.message.'.(int) $Id;
		};

		$actions = JAccess::getActions('com_multimidias', 'component');
	...
	public static function imageUpload($file, $data, $fieldname)
	{
		...
		require_once('components/com_multimidias/assets/wideimage/WideImage.php');

		// Set message array
		$info = array();

		// Set folders
		$mediaFolder	= JPATH_SITE . DS . 'images' . DS . 'com_multimidias';
