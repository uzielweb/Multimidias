...
/**
 * Multimidias Controller Multimidia
 */
class MultimidiasControllermultimidia extends JControllerForm
{
	public function __construct($config = array())
	{
		$this->view_list = 'multimidias'; // safeguard for setting the return view listing to the main view.
	...
	protected function postSaveHook(JModelLegacy &$model, $validData = array())
	{
		...
		if(array_key_exists('thumbnail_delete', $_POST)){
			...
			$db = JFactory::getDBO();
			$query = $db->getQuery(true)->select('thumbnail')->from('#__multimidias_multimidia')->where('id="' . $validData['id'] . '"');
			$db->setQuery((string)$query);
			$thumbnail = $db->loadResult();
			if($thumbnail){
				$query = $db->getQuery(true)->select('CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END')->from('#__multimidias_multimidia')->where('thumbnail="' . $thumbnail . '" AND id!="' . $validData['id'] . '"');
				$db->setQuery((string)$query);
				$using = $db->loadResult();
