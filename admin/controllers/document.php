 ...
 */
class MultimidiasControllerdocument extends JControllerForm
{
	public function __construct($config = array())
	{
		$this->view_list = 'documents'; // safeguard for setting the return view listing to the main view.
	...
	protected function postSaveHook(JModelLegacy &$model, $validData = array())
	{
		...
		if(array_key_exists('file_delete', $_POST)){
			...
			$db = JFactory::getDBO();
			$query = $db->getQuery(true)->select('file')->from('#__multimidias_document')->where('id="' . $validData['id'] . '"');
			$db->setQuery((string)$query);
			$file = $db->loadResult();
			if($file){
				$query = $db->getQuery(true)->select('CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END')->from('#__multimidias_document')->where('file="' . $file . '" AND id!="' . $validData['id'] . '"');
				$db->setQuery((string)$query);
				$using = $db->loadResult();
